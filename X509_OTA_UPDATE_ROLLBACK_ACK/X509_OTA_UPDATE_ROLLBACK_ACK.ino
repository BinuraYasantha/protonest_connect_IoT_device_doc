...
const char* CLIENT_CERT_PATH = "/device-cert.pem";  // Client Certificate path (from downloaded zip file) device-cert.pem
const char* CLIENT_KEY_PATH = "/device-key.pem";    // Root CA Certificate path (from downloaded zip file) device-key.pem
...
// Connect to configured WiFi SSID (simple blocking connect with timeout)
// --- ROBUST WIFI CONNECT ---
bool connectWifi() {
...
// Load the certificates from LittleFS (if present) into the secure client
